...
Q3: Why does JAX occupy GPU memory before training starts?

A3: The training and policy-serving entry points disable JAX GPU-memory preallocation by default. JAX allocates only the memory it needs and releases unused allocations via the platform allocator. For custom entry points, set `XLA_PYTHON_CLIENT_PREALLOCATE=false` and `XLA_PYTHON_CLIENT_ALLOCATOR=platform` before importing JAX.
