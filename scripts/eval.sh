...
EVAL_SEEDS_CSV=$(IFS=,; printf '%s' "${SEEDS[*]}")
SERVER_PID=""
...
run_evaluation() {
    ...
    server_log="$SERVER_LOG_DIR/${POLICY_NAME}_ckpt${CKPT_ID}_seed${model_seed}_repeat${repeat_id}_port${eval_port}.log"

    echo "======================================================================"
    echo "Starting seed $model_seed, repeat $repeat_id/$NUM_REPEATS"
    echo "Result directory: $SAVE_DIR/$POLICY_NAME/${EVAL_RUN_NAME:-default}/seed$model_seed/repeat$repeat_id"
    echo "Policy endpoint: $HOST:$eval_port"
    ...
    start_time=$SECONDS
    until lsof -iTCP:"$eval_port" -sTCP:LISTEN &>/dev/null; do
        if ! kill -0 "$SERVER_PID" 2>/dev/null; then
...
for model_seed in "${SEEDS[@]}"; do
    for ((repeat_id = 1; repeat_id <= NUM_REPEATS; repeat_id++)); do
        run_evaluation "$model_seed" "$repeat_id"
    done
done

echo
echo "All evaluations completed."
echo "Aggregate summary: $SAVE_DIR/$POLICY_NAME/${EVAL_RUN_NAME:-default}/summary.json"
