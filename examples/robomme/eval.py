...
def get_evaluation_run_directory(args: Args) -> Path:
    ...
    return Path(args.save_dir) / args.policy_name / get_evaluation_run_name(args)
...
def update_run_summary(args: Args) -> Path:
    ...
    seed_values = [
        int(value.strip())
        for value in (args.eval_seeds or str(args.model_seed)).split(",")
        if value.strip()
    ]
    if not seed_values:
...
def evaluate(args: Args):
    ...
    while not os.path.exists(save_dir / "log.json"):
        ...
        try:
            ...
            final_results["model_seed"] = args.model_seed
            final_results["repeat_id"] = args.repeat_id
            final_results["model_ckpt_id"] = args.model_ckpt_id
